...
export function canonicalTitle(raw: string): string {
  ...
  for (let i = 0; i < 3; i++) {
    s = s
      ...
      .replace(/\s*\([^)]*(?:\b\d[\d./,\-x]*\b|v\s*[\d.]+|b\s*\d+|build|release|update|patch|hot\s?fix|early access|alpha|beta|eng|ger|rus|multi|\bdlc\b|soundtrack|ost|bonus|digital content|сабами|субтитрами|озвучка|русификатор|русский)[^)]*\)?\s*$/gi, "")
