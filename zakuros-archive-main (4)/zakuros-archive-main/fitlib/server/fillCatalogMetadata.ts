...
async function main() {
  ...
  for (const game of candidates) {
    ...
    if (Date.now() - lastCatalogSave > 15000) {
      saveCatalog(games);
      saveState({
        attemptedMatch: Array.from(attemptedMatch),
        protonDone: Array.from(protonDone),
      });
  ...
  console.log(
    `[Grind] done: ${bDone} appids visited | +${bEnriched} proton tiers | catalog now has Linux support for ${withLinux} games`
  );
